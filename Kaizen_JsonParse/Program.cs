...
class Program
{
   ...
   },
   {
      ...
      ""boundingPoly"":{
        ...
        // JSON response'ı parse etme
        JArray responseArray = JArray.Parse(jsonResponse);

        // Koordinatlara göre metin parçalarını sıralama
        List<JObject> sortedItems = responseArray.ToObject<List<JObject>>();
        sortedItems.Sort((a, b) => CompareByCoordinates(a, b));

        // Sıralanmış metin parçalarını işleme
        foreach (JObject item in sortedItems)
        {
            string description = item["description"].ToString();
            Console.WriteLine("Metin: " + description);
        }
    }

    // Koordinatlara göre sıralama fonksiyonu
    static int CompareByCoordinates(JObject a, JObject b)
    {
        var verticesA = a["boundingPoly"]["vertices"];
        var verticesB = b["boundingPoly"]["vertices"];

        int minXa = verticesA.Min(v => (int)v["x"]);
        int minYa = verticesA.Min(v => (int)v["y"]);
        int minXb = verticesB.Min(v => (int)v["x"]);
        int minYb = verticesB.Min(v => (int)v["y"]);
